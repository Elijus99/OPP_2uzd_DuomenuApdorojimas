...
{
	std::deque<stud> minksti;
	std::deque<stud>::size_type i = 0;
	int num = 0;
	while (i != ls.size()) {
		if (ls[i].galutinis < 5.0) {
			minksti.push_back(ls[i]);
			++i;
		}
		else {
			ls.push_front(ls[i]);
...
void OutputToFiles(std::deque<stud> &ls, std::deque<stud> minksti, string GType, int VSize, int PSize)
{
	...
	{
		outToV << std::left << setw(VSize + 6) << minksti[i].vard << setw(PSize + 7) << minksti[i].pav << std::fixed << setprecision(2) << minksti[i].galutinis << endl;
	}
