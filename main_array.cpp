...
void ivedimas(char &GType, std::vector<stud> &ls, int &VSize, int &PSize)
{
	...
	std::mt19937 mt(rd());
	std::uniform_int_distribution <> gen(1, std::nextafter(10, DBL_MAX));
	for (int i = 0; i < N; i++)
	{
		ls.push_back(stud());
		...
		rikiavimas(ls[i].C, ls[i].n);

		if (ls[i].n % 2 == 1) {
			mediana = ls[i].C[ls[i].n / 2];
		}
		else {
			mediana = (double)(ls[i].C[ls[i].n / 2 - 1] + ls[i].C[ls[i].n / 2]) / 2;
		}
